...
export function Checkable({ type='checkbox', label='', onChange=()=>{}, style={}, children, className='', name='', disabled, message, error, warning, ...rest }) {
	let _type = type === 'toggle' ? 'checkbox' : type;

	let classes = Object.entries({ error, disabled, warning }).filter(([k, v]) => v).map(([k]) => Style[k]).join(' ');
	...
	return (
		<label style={style} type={type} className={_class}>
			...
			<span className={`checkable-mark ${type}`}/>
